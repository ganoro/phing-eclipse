...
public class FileUtils {
	 ...
	 * granularity of 100 nanoseconds, which is less than 1 millisecond, so we
	 * round this up to 1 millisecond.
	 */
	public static final long NTFS_FILE_TIMESTAMP_GRANULARITY = 1;

	/**
	 * A one item cache for fromUri. fromUri is called for each element when
	 * parseing ant build files. It is a costly operation. This just caches the
	 ...
	 * @deprecated since 1.7. Use getFileUtils instead, FileUtils do not have
	 *             state.
	 */
	 ...
	 */
	public void copyFile(String sourceFile, String destFile,
			FilterSetCollection filters, boolean overwrite) throws IOException {
	 ...
	 * used, if source files may overwrite newer destination files and the last
	...
	public void copyFile(String sourceFile, String destFile,
			...
			boolean overwrite, boolean preserveLastModified, String encoding,
	...
	public void copyFile(String sourceFile, String destFile,
			...
			throws IOException {
		copyFile(new File(sourceFile), new File(destFile), filters,
				filterChains, overwrite, preserveLastModified, inputEncoding,
	 ...
	 * Convenience method to copy a file from a source to a destination
	 * specifying if token filtering must be used, if source files may overwrite
	 * newer destination files, the last modified time of <code>destFile</code>
	...
	public void copyFile(File sourceFile, File destFile,
		...
		copyFile(sourceFile, destFile, filters, filterChains, overwrite,
				preserveLastModified, encoding, encoding, project);
	}
	...
	public void copyFile(File sourceFile, File destFile,
			...
			throws IOException {
		copyFile(sourceFile, destFile, filters, filterChains, overwrite,
				preserveLastModified, false, inputEncoding, outputEncoding,
				project);
	}
	...
	public void copyFile(File sourceFile, File destFile,
		...
		ResourceUtils.copyResource(new FileResource(sourceFile),
				new FileResource(destFile), filters, filterChains, overwrite,
				preserveLastModified, append, inputEncoding, outputEncoding,
				project, force);
	}
	...
	/**
	 * Calls File.setLastModified(long time). Originally written to to
	 * dynamically bind to that call on Java1.2+.
	 * 
	 * @param file
	 *            the file whose modified time is to be set
	 * @param time
	 *            the time to which the last modified time is to be set. if this
	 *            is -1, the current time is used.
	 */
	 ...
	 * <code>new File(file, filename)</code> in that the resulting File's path
	 * will always be a normalized, absolute pathname. Also, if it is determined
	 * that <code>filename</code> is context-relative, <code>file</code> will be
	 * discarded and the reference will be resolved using available
	 * context/state information about the filesystem.
	 * 
	 * @param file
	 ...
	 * (relative to current root directory) and filenames with a drive
	 * specification and no intervening separator (relative to current directory
	...
	public static boolean isContextRelativePath(String filename) {
		...
		return (c == sep && (len == 1 || filename.charAt(1) != sep))
				|| (Character.isLetter(c) && len > 1
						&& filename.indexOf(':') == 1 && (len == 2 || filename
						.charAt(2) != sep));
	}

	/**
	 * Verifies that the specified filename represents an absolute path. Differs
	 * from new java.io.File("filename").isAbsolute() in that a path beginning
	 * with a double file separator--signifying a Windows UNC--must at minimum
	 * match "\\a\b" to be considered an absolute path.
	 * 
	 * @param filename
	...
	public static boolean isAbsolutePath(String filename) {
		...
		return (Character.isLetter(c) && colon == 1 && filename.length() > 2 && filename
				.charAt(2) == sep) || (ON_NETWARE && colon > 0);
	}
	...
	/**
	 * Returns a VMS String representation of a <code>File</code> object. This
	 * is useful since the JVM by default internally converts VMS paths to Unix
	 ...
	 * @param createFile
	 *            true if the file must actually be created. If false chances
	 *            exist that a file with the same name is created in the time
	 *            between invoking this method and the moment the file is
	 *            actually created. If possible set to true.
	 * 
	...
	public boolean contentEquals(File f1, File f2, boolean textfile)
			throws IOException {
		return ResourceUtils.contentEquals(new FileResource(f1),
				new FileResource(f2), textfile);
	}

	/**
	 * This was originally an emulation of {@link File#getParentFile} for JDK
	 * 1.1, but it is now implemented using that method (Ant 1.6.3 onwards).
	 * 
	 * @param f
	...
	/**
	 * This was originally an emulation of File.createNewFile for JDK 1.1, but
	 * it is now implemented using that method (Ant 1.6.3 onwards).
	 * 
	 * <p>
	 * This method has historically <strong>not</strong> guaranteed that the
	...
	/**
	 * Constructs a <code>file:</code> URI that represents the external form of
	 * the given pathname.
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * The coding of the output is the same as what File.toURI().toASCIIString()
	 * produces
	 * </p>
	 * 
	 * See <a href="http://www.w3.org/TR/xml11/#dt-sysid">dt-sysid</a> which
	 * makes some mention of how characters not supported by URI Reference
	...
	public String fromURI(String uri) {
		synchronized (cacheFromUriLock) {
			...
			String ret = isAbsolutePath(path) ? normalize(path)
	 ...
	 * 
	 * <p>
	 * Unlike java.io.File#equals this method will try to compare the absolute
	...
	/**
	 * Are the two File instances pointing to the same object on the file
	 * system?
	 * 
	 * @since Ant 1.8.2
	 */
	public boolean areSame(File f1, File f2) throws IOException {
		...
		return f1Normalized.equals(f2Normalized)
				|| f1Normalized.getCanonicalFile().equals(
						f2Normalized.getCanonicalFile());
	}
	 ...
	 * which is incorrect--it should really be by filesystem. We do not have an
	 * easy way to probe for file systems, however, so this heuristic gives us a
	 * decent default.
	 * 
	 * @return the difference, in milliseconds, which two file timestamps must
	 *         have in order for the two files to be considered to have
	...
	/**
	 * test whether a file or directory exists, with an error in the upper/lower
	 * case spelling of the name. Using this method is only interesting on case
	 * insensitive file systems (Windows).<br/>
	...
	public boolean hasErrorInCase(File localFile) {
		...
		FilenameFilter ff = new FilenameFilter() {
			public boolean accept(File dir, String name) {
				return name.equalsIgnoreCase(localFileName)
	...
	/**
	 * Returns true if the source is older than the dest. If the dest file does
	 * not exist, then the test returns false; it is implicitly not up do date.
	 * 
	 * @param source
	 ...
	 * @return true if the source is older than the dest after accounting for
	 *         granularity.
	 * @since Ant 1.6.3
	 */
	 ...
	 * @param dest
	 *            dest file (should be the newer).
	 * @return true if the source is older than the dest, taking the granularity
	 *         into account.
	 * @since Ant 1.6.3
	 */
	...
	/**
	 * Compare two timestamps for being up to date using the specified
	 * granularity.
	 * 
	 * @param sourceTime
	...
	/**
	 * Compare two timestamps for being up to date using the current
	 * granularity.
	 * 
	 * @param sourceTime
	 *            timestamp of source file.
	...
	/**
	 * Close a Writer without throwing any exception if something went wrong. Do
	 * not attempt to close it if the argument is null.
	 * 
	 * @param device
	...
	/**
	 * Close a Reader without throwing any exception if something went wrong. Do
	 * not attempt to close it if the argument is null.
	 * 
	 * @param device
	 *            Reader, can be null.
	...
	/**
	 * Close a stream without throwing any exception if something went wrong. Do
	 * not attempt to close it if the argument is null.
	 * 
	 * @param device
	 *            stream, can be null.
	...
	/**
	 * Closes an URLConnection if its concrete implementation provides a way to
	 * close it that Ant knows of.
	 * 
	 * @param conn
	 ...
	 * This function may throw an IOException if an I/O error occurs because its
	 * use of the canonical pathname may require filesystem queries.
	 * </p>
	 * 
	 * @param fromFile
	 ...
	 * 
	 * @param pathStack
	 *            <code>List</code> of <code>String</code>s to be concatenated
	 *            as a path.
	 ...
	 * 
	 * @param pathStack
	 *            <code>List</code> of <code>String</code>s to be concated as a
	 *            path.
	 * @param separatorChar
	...
	/**
	 * Get the default encoding. This is done by opening an InputStreamReader on
	 * a dummy InputStream and getting the encoding. Could use
	 * System.getProperty("file.encoding"), but cannot see where this is
	 * documented.
	 * 
