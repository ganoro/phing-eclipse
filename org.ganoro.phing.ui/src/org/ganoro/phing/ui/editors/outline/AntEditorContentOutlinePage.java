...
import org.ganoro.phing.ui.internal.model.AntImportNode;
import org.ganoro.phing.ui.internal.model.AntModel;
import org.ganoro.phing.ui.internal.model.AntModelChangeEvent;
import org.ganoro.phing.ui.internal.model.AntModelContentProvider;
import org.ganoro.phing.ui.internal.model.AntModelCore;
import org.ganoro.phing.ui.internal.model.AntModelLabelProvider;
import org.ganoro.phing.ui.internal.model.AntProjectNode;
import org.ganoro.phing.ui.internal.model.AntPropertyNode;
...
public class AntEditorContentOutlinePage extends ContentOutlinePage implements IShowInSource, IAdaptable {
	...
	private void setFilter(boolean filter, String name) {
		if (name != null) {
			PhingUi.getDefault().getPreferenceStore().setValue(name, filter);
		}
	...
	protected void setSort(boolean sort) {
		...
		PhingUi.getDefault().getPreferenceStore().setValue(IAntUIPreferenceConstants.ANTEDITOR_SORT, sort);
	}
	 ...
	 */
	public AntEditorContentOutlinePage(AntModelCore core, PhingEditor editor) {
		super();
		fCore= core;
		fFilterInternalTargets= PhingUi.getDefault().getPreferenceStore().getBoolean(IAntUIPreferenceConstants.ANTEDITOR_FILTER_INTERNAL_TARGETS);
		fFilterImportedElements= PhingUi.getDefault().getPreferenceStore().getBoolean(IAntUIPreferenceConstants.ANTEDITOR_FILTER_IMPORTED_ELEMENTS);
		fFilterProperties= PhingUi.getDefault().getPreferenceStore().getBoolean(IAntUIPreferenceConstants.ANTEDITOR_FILTER_PROPERTIES);
		fFilterTopLevel= PhingUi.getDefault().getPreferenceStore().getBoolean(IAntUIPreferenceConstants.ANTEDITOR_FILTER_TOP_LEVEL);
		fSort= PhingUi.getDefault().getPreferenceStore().getBoolean(IAntUIPreferenceConstants.ANTEDITOR_SORT);
		fEditor= editor;
