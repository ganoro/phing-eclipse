 ...
 */
public class PhingCorePreferences implements org.eclipse.core.runtime.Preferences.IPropertyChangeListener {
	...
	static private class Relation {
		...
		public String toString() {
			return from.toString() + "->" + (to == null ? IPhingCoreConstants.EMPTY_STRING : to.toString()); //$NON-NLS-1$
		}
	}
	...
	protected PhingCorePreferences(List defaultTasks, List defaultExtraClasspath, List defaultTypes, boolean headless) {
		this(defaultTasks, defaultExtraClasspath, defaultTypes, Collections.EMPTY_LIST, headless);
	}
	
	protected PhingCorePreferences(List defaultTasks, List defaultExtraClasspath, List defaultTypes, List defaultProperties, boolean headless) {
	...
	public void propertyChange(Preferences.PropertyChangeEvent event) {
		Preferences prefs = PhingCore.getPlugin().getPluginPreferences();
		String property= event.getProperty();
		if (property.equals(IPhingCoreConstants.PREFERENCE_TASKS) || property.startsWith(IPhingCoreConstants.PREFIX_TASK)) {
			restoreTasks(prefs);
		} else if (property.equals(IPhingCoreConstants.PREFERENCE_TYPES) || property.startsWith(IPhingCoreConstants.PREFIX_TYPE)) {
			restoreTypes(prefs);
		} else if (property.equals(IPhingCoreConstants.PREFERENCE_ANT_HOME_ENTRIES)) {
			restoreAntHomeEntries(prefs);
		} else if (property.equals(IPhingCoreConstants.PREFERENCE_ADDITIONAL_ENTRIES)) {
			restoreAdditionalEntries(prefs);
		} else if (property.equals(IPhingCoreConstants.PREFERENCE_ANT_HOME)) {
			restoreAntHome(prefs);
		} else if (property.equals(IPhingCoreConstants.PREFERENCE_PROPERTIES) || property.startsWith(IPhingCoreConstants.PREFIX_PROPERTY)) {
			restoreCustomProperties(prefs);
		} else if (property.equals(IPhingCoreConstants.PREFERENCE_PROPERTY_FILES)) {
	...
	private void restoreTasks(Preferences prefs) {
		 String tasks = prefs.getString(IPhingCoreConstants.PREFERENCE_TASKS);
		 if (tasks.equals(IPhingCoreConstants.EMPTY_STRING)) {
	...
	private void restoreTypes(Preferences prefs) {
		String types = prefs.getString(IPhingCoreConstants.PREFERENCE_TYPES);
		if (types.equals(IPhingCoreConstants.EMPTY_STRING)) {
	...
	private void restoreAntHomeEntries(Preferences prefs) {
		...
		if (entries.equals(IPhingCoreConstants.EMPTY_STRING)) {
			entries= prefs.getString(IPhingCoreConstants.PREFERENCE_ANT_HOME_ENTRIES);
		} else {
	...
	private void restoreAdditionalEntries(Preferences prefs) {
		...
		if (entries.equals(IPhingCoreConstants.EMPTY_STRING)) {
			entries = prefs.getString(IPhingCoreConstants.PREFERENCE_ADDITIONAL_ENTRIES);
		} else {
	...
	private void restoreAntHome(Preferences prefs) {
		antHome= prefs.getString(IPhingCoreConstants.PREFERENCE_ANT_HOME);
	...
	private void restoreCustomProperties(Preferences prefs) {
		String properties = prefs.getString(IPhingCoreConstants.PREFERENCE_PROPERTIES);
		if (properties.equals(IPhingCoreConstants.EMPTY_STRING)) {
	...
	private void restoreCustomPropertyFiles(Preferences prefs) {
		String propertyFiles= prefs.getString(IPhingCoreConstants.PREFERENCE_PROPERTY_FILES);
		if (propertyFiles.equals(IPhingCoreConstants.EMPTY_STRING)) {
	...
	protected Task[] extractTasks(Preferences prefs, String[] tasks) {
		...
		for (int i = 0; i < tasks.length; i++) {
			...
			String[] values = getArrayFromString(prefs.getString(IPhingCoreConstants.PREFIX_TASK + taskName));
			if (values.length < 2) {
				continue;
			}
			Task task = new Task();
			...
			if (library.startsWith(IPhingCoreConstants.FILE_PROTOCOL)) {
	...
	protected Type[] extractTypes(Preferences prefs, String[] types) {
		...
		for (int i = 0; i < types.length; i++) {
			...
			String[] values = getArrayFromString(prefs.getString(IPhingCoreConstants.PREFIX_TYPE + typeName));
			if (values.length < 2) {
				continue;
			}
			Type type = new Type();
	...
	protected Property[] extractProperties(Preferences prefs, String[] properties) {
		...
		for (int i = 0; i < properties.length; i++) {
			...
			String[] values = getArrayFromString(prefs.getString(IPhingCoreConstants.PREFIX_PROPERTY + propertyName));
			if (values.length < 1) {
				continue;
			}
	...
	public synchronized IAntClasspathEntry[] getDefaultAntHomeEntries() {
		if (defaultAntHomeEntries == null) {
			ServiceTracker tracker = new ServiceTracker(PhingCore.getPlugin().getBundle().getBundleContext(), PackageAdmin.class.getName(), null);
			tracker.open();
			try {
					...
					if(bundle == null) {
						for (int i = 0; i < packages.length; i++) {
							...
							try {
								...
								PhingCore.log(ioe); // maintain logging
								result.clear();
	...
	protected List computeDefaultTasks(List tasks) {
		...
		for (Iterator iterator = tasks.iterator(); iterator.hasNext();) {
			...
			Task task = new Task();
			task.setTaskName(element.getAttribute(IPhingCoreConstants.NAME));
			task.setClassName(element.getAttribute(PhingCore.CLASS));
	...
	private void addURLToExtraClasspathEntries(URL url, IConfigurationElement element) {
		String eclipseRuntime= element.getAttribute(PhingCore.ECLIPSE_RUNTIME);
	...
	protected List computeDefaultTypes(List types) {
		...
		for (Iterator iterator = types.iterator(); iterator.hasNext();) {
			...
			Type type = new Type();
			type.setTypeName(element.getAttribute(IPhingCoreConstants.NAME));
			type.setClassName(element.getAttribute(PhingCore.CLASS));
	...
	private URL getClasspathEntryURL(Bundle bundle, String library) throws IOException {
	    ...
	    return new URL(IPhingCoreConstants.FILE_PROTOCOL + (urlFile.isDirectory() ? path + "/" : path));  //$NON-NLS-1$
	}
	...
	private void configureAntObject(List result, IConfigurationElement element, AntObject antObject, String objectName, String errorMessage) {
		String runtime = element.getAttribute(PhingCore.ECLIPSE_RUNTIME);
		if (runtime != null) {
			antObject.setEclipseRuntimeRequired(Boolean.valueOf(runtime).booleanValue());
		}
		
		String uri = element.getAttribute(PhingCore.URI);
		if (uri != null) {
			antObject.setURI(uri);
		}
		
		String library = element.getAttribute(PhingCore.LIBRARY);
		if (library == null) {
			IStatus status = new Status(IStatus.ERROR, PhingCore.PI_ANTCORE, PhingCore.ERROR_LIBRARY_NOT_SPECIFIED, NLS.bind(InternalCoreAntMessages.AntCorePreferences_Library_not_specified_for___0__4, new String[]{objectName}), null);
			PhingCore.getPlugin().getLog().log(status);
			return;
		}
		...
		try {
			...
			IStatus status = new Status(IStatus.ERROR, PhingCore.PI_ANTCORE, PhingCore.ERROR_LIBRARY_NOT_SPECIFIED, NLS.bind(errorMessage, new String[]{library, element.getContributor().getName()}), null);
			PhingCore.getPlugin().getLog().log(status);
			return;
		} catch (MalformedURLException e) {
			// if the URL does not have a valid format, just log and ignore the exception
			IStatus status = new Status(IStatus.ERROR, PhingCore.PI_ANTCORE, PhingCore.ERROR_MALFORMED_URL, InternalCoreAntMessages.AntCorePreferences_Malformed_URL__1, e);
			PhingCore.getPlugin().getLog().log(status);
			return;
		} catch (Exception e) {
			//likely extra classpath entry library that does not exist
			IStatus status = new Status(IStatus.ERROR, PhingCore.PI_ANTCORE, PhingCore.ERROR_LIBRARY_NOT_SPECIFIED, NLS.bind(InternalCoreAntMessages.AntCorePreferences_8, new String[]{library,  element.getContributor().getName()}), null);
			PhingCore.getPlugin().getLog().log(status);
			return;
		}
	}
		...
		for (Iterator iterator = entries.iterator(); iterator.hasNext();) {
			...
			try {
				...
				if (url != null) {
					...
					IStatus status = new Status(IStatus.ERROR, PhingCore.PI_ANTCORE, PhingCore.ERROR_LIBRARY_NOT_SPECIFIED, NLS.bind(InternalCoreAntMessages.AntCorePreferences_6, new String[]{library, element.getContributor().getName()}), null);
					PhingCore.getPlugin().getLog().log(status);
					continue;
				}
			} catch (MalformedURLException e) {
	...
	private boolean relevantRunningHeadless(IConfigurationElement element) {
		if (runningHeadless) {
			String headless = element.getAttribute(PhingCore.HEADLESS);
			if (headless != null) {
	...
	private void computeDefaultProperties(List properties) {
		...
		for (Iterator iterator = properties.iterator(); iterator.hasNext();) {
			...
			String name = element.getAttribute(IPhingCoreConstants.NAME);
			if (name == null) {
				continue;
			}
			String value = element.getAttribute(IPhingCoreConstants.VALUE);
			...
			if (value != null) {
				...
				property.setPluginLabel(element.getContributor().getName());
				String className = element.getAttribute(PhingCore.CLASS);
	...
	private void addLibraries(Bundle source, List destination) throws IOException, MalformedURLException {
		...
		try {
			libraries = ManifestElement.parseHeader(Constants.BUNDLE_CLASSPATH, (String) source.getHeaders(IPhingCoreConstants.EMPTY_STRING).get(Constants.BUNDLE_CLASSPATH));
		} catch (BundleException e) {
			IStatus status = new Status(IStatus.ERROR, PhingCore.PI_ANTCORE, PhingCore.ERROR_MALFORMED_URL, InternalCoreAntMessages.AntCorePreferences_0, e);
			PhingCore.getPlugin().getLog().log(status);
			return;
		}
		if (libraries == null) {
	...
	private List computePrerequisiteOrder(List plugins) {
		...
		for (Iterator iter = fragments.iterator(); iter.hasNext();) {
			...
			if (plugins.contains(current.getHost().getBundle())) {
				...
				PhingCore.getPlugin().getLog().log(new Status(IStatus.ERROR, PhingCore.PI_ANTCORE, PhingCore.ERROR_MALFORMED_URL, NLS.bind(InternalCoreAntMessages.AntCorePreferences_1, new String[] {current.getSymbolicName()}), null));
			}
	...
	private void initializePluginClassLoaders() {
		...
		pluginClassLoaders.add(new WrappedClassLoader(PhingCore.getPlugin().getBundle()));
	}
	...
	protected String[] getArrayFromString(String list) {
		...
		if (list == null || list.trim().equals(IPhingCoreConstants.EMPTY_STRING)) {
			return new String[0];
		}
		ArrayList result = new ArrayList();
		for (StringTokenizer tokens = new StringTokenizer(list, separator); tokens.hasMoreTokens();) {
			String token = tokens.nextToken().trim();
			if (!token.equals(IPhingCoreConstants.EMPTY_STRING)) {
				result.add(token);
			}
		}
	...
	public void updatePluginPreferences() {
		...
		boolean classpathChanged= PhingCore.getPlugin().getPluginPreferences().needsSaving();
		PhingCore.getPlugin().savePluginPreferences();
		if (classpathChanged) {
			prefs.setValue(IPhingCoreConstants.PREFERENCE_CLASSPATH_CHANGED, true);
		}
		prefs.setValue(IPhingCoreConstants.PREFERENCE_CLASSPATH_CHANGED, false);
		prefs.addPropertyChangeListener(this);
	}
	...
	protected void updateTasks(Preferences prefs) {
		if (oldCustomTasks != null) {
			for (int i = 0; i < oldCustomTasks.length; i++) {
				...
				prefs.setToDefault(IPhingCoreConstants.PREFIX_TASK + oldTask.getTaskName());
			}
			oldCustomTasks= null;	
		}	
		
		if (customTasks.length == 0) {
			prefs.setValue(IPhingCoreConstants.PREFERENCE_TASKS, IPhingCoreConstants.EMPTY_STRING);
			return;
		}
		...
		for (int i = 0; i < customTasks.length; i++) {
			...
			tasks.append(',');
			prefs.setValue(IPhingCoreConstants.PREFIX_TASK + customTasks[i].getTaskName(), customTasks[i].getClassName() + "," + customTasks[i].getLibraryEntry().getLabel()); //$NON-NLS-1$
		}
		prefs.setValue(IPhingCoreConstants.PREFERENCE_TASKS, tasks.toString());
	}
	...
	protected void updateTypes(Preferences prefs) {
		if (oldCustomTypes != null) {
			for (int i = 0; i < oldCustomTypes.length; i++) {
				...
				prefs.setToDefault(IPhingCoreConstants.PREFIX_TYPE + oldType.getTypeName());
			}
			oldCustomTypes= null;	
		}	
				
		if (customTypes.length == 0) {
			prefs.setValue(IPhingCoreConstants.PREFERENCE_TYPES, IPhingCoreConstants.EMPTY_STRING);
			return;
		}
		...
		for (int i = 0; i < customTypes.length; i++) {
			...
			types.append(',');
			prefs.setValue(IPhingCoreConstants.PREFIX_TYPE + customTypes[i].getTypeName(), customTypes[i].getClassName() + "," + customTypes[i].getLibraryEntry().getLabel()); //$NON-NLS-1$
		}
		prefs.setValue(IPhingCoreConstants.PREFERENCE_TYPES, types.toString());
	}
	...
	protected void updateProperties(Preferences prefs) {
		if (oldCustomProperties != null) {
			for (int i = 0; i < oldCustomProperties.length; i++) {
				...
				prefs.setToDefault(IPhingCoreConstants.PREFIX_PROPERTY + oldProperty.getName());
			}
			oldCustomProperties= null;
		}
		
		if (customProperties.length == 0) {
			prefs.setValue(IPhingCoreConstants.PREFERENCE_PROPERTIES, IPhingCoreConstants.EMPTY_STRING);
			return;
		}
		...
		for (int i = 0; i < customProperties.length; i++) {
			...
			properties.append(',');
			prefs.setValue(IPhingCoreConstants.PREFIX_PROPERTY + customProperties[i].getName(), customProperties[i].getValue(false));
		}
		prefs.setValue(IPhingCoreConstants.PREFERENCE_PROPERTIES, properties.toString());
	}

	protected void updateAdditionalEntries(Preferences prefs) {
		prefs.setValue("urls", IPhingCoreConstants.EMPTY_STRING); //old constant removed  //$NON-NLS-1$
		String serialized= IPhingCoreConstants.EMPTY_STRING;
		IAntClasspathEntry toolsJarEntry= getToolsJarEntry();
		List userLibs= getUserLibraries();
		if (userLibs == null) {
		...
		prefs.setValue(IPhingCoreConstants.PREFERENCE_ADDITIONAL_ENTRIES, serialized);
		
		String prefAntHome= IPhingCoreConstants.EMPTY_STRING;
		if (antHome != null && !antHome.equals(getDefaultAntHome())) {
			prefAntHome= antHome;
		} 
		prefs.setValue(IPhingCoreConstants.PREFERENCE_ANT_HOME, prefAntHome);
	}
	
	protected void updateAntHomeEntries(Preferences prefs) {
		prefs.setValue("ant_urls", IPhingCoreConstants.EMPTY_STRING); //old constant removed  //$NON-NLS-1$
		...
		if (dflt) {
			...
			prefs.setValue(IPhingCoreConstants.PREFERENCE_ANT_HOME_ENTRIES, IPhingCoreConstants.EMPTY_STRING);
			return;
		}
		StringBuffer entries = new StringBuffer();
		for (int i = 0; i < antHomeEntries.length; i++) {
		...
		prefs.setValue(IPhingCoreConstants.PREFERENCE_ANT_HOME_ENTRIES, entries.toString());
	}
	...
	protected void updatePropertyFiles(Preferences prefs) {
		...
		prefs.setValue(IPhingCoreConstants.PREFERENCE_PROPERTY_FILES, files.toString());
	}
